...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-5xl mb-8 flex justify-end items-center no-print">
        <div className="flex gap-4">
          <button 
            onClick={handleDownloadPDF}
            disabled={isDownloading}
            className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm font-medium text-base disabled:bg-blue-400 disabled:cursor-not-allowed"
          >
            {isDownloading ? (
              <>
      ...
      {/* Quote Document */}
      <div id="quote-container" className="w-full max-w-5xl bg-white shadow-xl print-shadow-none rounded-xl p-10 md:p-14 text-slate-800 print:p-0">
        
        <Header data={clientData} />

        {/* Intro */}
        <div className="mb-12 text-slate-600 leading-relaxed text-lg">
          <p className="mb-6">
          ...
          {clientData.websiteLink && (
            <div className="my-8 p-4 bg-blue-50 border-l-4 border-blue-600 rounded-r-lg">
              <p className="font-bold text-blue-900 mb-2">Live Demo Available</p>
              <p className="text-blue-800 mb-2">I have prepared a live preview of the proposed design. You can explore it here:</p>
              <a 
                href={clientData.websiteLink}
                target="_blank" 
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-blue-700 font-bold hover:text-blue-900 hover:underline text-lg"
              >
                {clientData.websiteLink} <ExternalLink className="w-5 h-5" />
              </a>
            </div>
          )}
        ...
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-8 mb-12 page-break-inside-avoid">
          <h3 className="text-xl font-bold text-slate-800 mb-6 border-b border-slate-200 pb-4">Financial Summary</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            
            {/* Card 1: Investment */}
            <div className="bg-white rounded-lg p-5 border border-slate-200 shadow-sm flex flex-col justify-center h-full">
              <p className="text-sm uppercase tracking-wide text-slate-500 font-bold mb-2">One-Time Investment</p>
              <div className="text-4xl font-bold text-slate-900">${pricingData.newCreation}</div>
              <p className="text-base text-slate-500 mt-2">Design & Development Fee</p>
            </div>
            
            {/* Card 2: Monthly */}
            <div className="bg-blue-50 rounded-lg p-5 border border-blue-100 shadow-sm flex flex-col justify-center h-full">
              <p className="text-sm uppercase tracking-wide text-blue-700 font-bold mb-2">New Monthly Total</p>
              <div className="text-4xl font-bold text-blue-700">${(pricingData.newHosting - pricingData.hostingDiscount) + pricingData.newMaintenance}</div>
              <p className="text-base text-blue-600/80 mt-2">Hosting + Maintenance</p>
            </div>

            {/* Card 3: Savings */}
            <div className="bg-green-50 rounded-lg p-5 border border-green-100 shadow-sm flex flex-col justify-center h-full">
              <p className="text-sm uppercase tracking-wide text-green-700 font-bold mb-2">Estimated Annual Savings</p>
              <div className="text-4xl font-bold text-green-700">${yearlySavings}</div>
              <p className="text-base text-green-600/80 mt-2">That's ${monthlySavings} saved every month</p>
            </div>
          </div>
          
          {pricingData.hostingDiscount > 0 && (
            <div className="mt-6 pt-6 border-t border-slate-200 text-base text-slate-500 flex items-center gap-3">
              <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-md text-sm font-bold">BONUS</span>
              I will cover the ${pricingData.newHosting}/mo hosting cost, making it free for you.
            </div>
          )}
        </div>

        {/* Closing & CTA */}
        <div className="mt-14 pt-10 border-t-2 border-slate-100 page-break-inside-avoid">
          ...
          <div className="flex flex-col sm:flex-row gap-10 items-start sm:items-center justify-between">
            <div>
              <p className="font-bold text-slate-900 text-xl">Ready to proceed?</p>
              <p className="text-slate-500 text-base mt-1">
                {isSigned ? "Quote accepted on " + new Date().toLocaleDateString() : "Please sign below to accept this quote."}
              </p>
            </div>
            
            <div className="w-full sm:w-72 pt-4">
              {isSigned && signatureData ? (
                <div className="border-b-2 border-slate-300 pb-2">
                  <img src={signatureData} alt="Client Signature" className="h-16 object-contain mb-2" />
                ...
                <div className="flex flex-col items-center">
                   <button 
                    ...
                    className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-md transition-all transform hover:scale-[1.02] active:scale-[0.98] no-print"
                  >
