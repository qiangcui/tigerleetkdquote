    ...
    <div className="mb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {benefits.map((benefit, index) => (
          <div key={index} className="flex items-start gap-5 p-6 rounded-xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="p-3 bg-slate-50 rounded-xl shrink-0">
              {benefit.icon}
            </div>
            <div>
              <h4 className="font-bold text-slate-900 mb-2 text-lg">{benefit.title}</h4>
